...
def get_reservations_data(field_name):
    ...
    football_field_reservations = get_batabase(query) 
...
def get_reservations_table(id):
    ...
    football_field_names = [name[0] for name in get_batabase(query)]
    reservations_table = []
    ...
    for football_field_name in football_field_names:
        ...
        reservations_table.append(get_batabase(query))
...
def get_my_reservations_table(id):
    ...
    reservations_table = get_batabase(query)
    return reservations_table

def delete_reservation(id):
    query = f"DELETE FROM hagz.reservations WHERE id = {id};"
    delete_database(query)
    return True